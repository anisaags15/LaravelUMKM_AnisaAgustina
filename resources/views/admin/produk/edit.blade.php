...
@section('title', 'Edit Produk - Admin DimsumYummy')
...
<div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-3xl font-extrabold text-[#2C1A12] tracking-tight">📝 Edit Produk</h1>
            <p class="text-stone-500 text-sm">Perbaharui informasi produk</p>
        </div>
    </div>

    {{-- Form --}}
    <form action="{{ route('admin.produk.update', $produk->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-8 rounded-3xl shadow-lg shadow-stone-200/50 border border-stone-100 space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="group space-y-2">
                ...
                <input type="text" name="nama" value="{{ old('nama', $produk->nama) }}" 
                       class="w-full border-2 border-stone-100 bg-stone-50/30 rounded-2xl p-4 focus:bg-white focus:ring-4 focus:ring-orange-100 focus:border-[#FB7E3C] outline-none transition-all" required>
            ...
            <div class="group space-y-2">
                ...
                <select name="id_kategori" class="w-full border-2 border-stone-100 bg-stone-50/30 rounded-2xl p-4 focus:bg-white focus:ring-4 focus:ring-orange-100 focus:border-[#FB7E3C] outline-none transition-all" required>
                    ...
                    @foreach($kategori as $kat)
                        <option value="{{ $kat->id }}" {{ old('id_kategori', $produk->kategori_id) == $kat->id ? 'selected' : '' }}>
                            🥟 {{ $kat->nama_kategori }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="group space-y-2">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="harga" value="{{ old('harga', $produk->harga) }}" 
                           class="w-full border-2 border-stone-100 bg-stone-50/30 rounded-2xl p-4 pl-12 focus:bg-white focus:ring-4 focus:ring-orange-100 focus:border-[#FB7E3C] outline-none transition-all" required>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="md:col-span-1 space-y-2">
                ...
                <div class="relative group cursor-pointer border-2 border-dashed border-orange-200 rounded-2xl bg-orange-50/20 hover:bg-orange-50/50 transition-all p-4 text-center">
                    ...
                    <div id="previewContainer" class="flex flex-col items-center justify-center space-y-2">
                        @if($produk->foto)
                            <img src="{{ asset('storage/' . $produk->foto) }}" id="fotoPreview" class="h-32 w-32 object-cover rounded-2xl border-4 border-white shadow-md">
                        @else
                ...
                <p class="text-xs text-stone-400">Kosongkan jika tidak ingin mengganti foto</p>
            ...
            <div class="md:col-span-2 space-y-2">
                ...
                <textarea name="deskripsi" rows="5" 
                          class="w-full border-2 border-stone-100 bg-stone-50/30 rounded-2xl p-4 focus:bg-white focus:ring-4 focus:ring-orange-100 focus:border-[#FB7E3C] outline-none transition-all" required>{{ old('deskripsi', $produk->deskripsi) }}</textarea>
        ...
        <div class="flex items-center justify-end pt-2">
            <label class="flex items-center cursor-pointer select-none">
                <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $produk->is_featured) ? 'checked' : '' }}
